...
T = [linspace(0.6,0.4,6) linspace(0.7,0.3,7) linspace(0.9,0.4,7)];
idx = [1 13 14 2 12 15 3 11 16 4 10 17 5 9 18 6 8 19 7 20];
S = S(idx);
T = T(idx);
W = [0.8 0.2];
alpha = 0.01;
...
for monte = 1:monte_carlo
    ...
    for ite = 1:max_ite
        ...
        for i = 1:nS
            ...
            n = connection(i);
            reward(i) = 0.3*(55/norm(USER(i,:)-UAV(n,:)))+0.6*(cpun(i)*CPU(n)/max(CPU))+0.1*(CPU(n)/max(CPU));
    ...
    name = 'results_' + string(round(now*1e5));
    mkdir(name);
